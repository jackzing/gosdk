...
//sign64bit to sign dgst
func sign64bit(dgst []byte, reader io.Reader, key []byte) ([]byte, uint8, error) {
	var rs [64]byte
	rr, ss := rs[:32], rs[32:]
...
//verifySignature64bit to verify a signature and return error
func verifySignature64bit(sig, dgst []byte, X []byte, Y []byte) (bool, error) {
	head := 0
