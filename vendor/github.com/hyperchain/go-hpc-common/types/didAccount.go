...
// DIDAccountAddrPrefix the prefix of didAccount in accountDB
var DIDAccountAddrPrefix = []byte{255, 255, 255, 255, 254}
...
func (did *DIDAccount) UnmarshalJSON(data []byte) error {
	...
	data = utils.DecodeString(string(data))
	if !bytes.HasPrefix(data, []byte(DIDPrefix)) {
...
func (did *DIDAccount) Hex() string {
	return "0x" + utils.BytesToHex(did.origin)
}
...
//IsDIDAddress return true if has did prefix
func IsDIDAddress(address []byte) bool {
	if bytes.HasPrefix(address, []byte(DIDPrefix)) {
		return true
	}
	return false
}

//IsInValidDIDAddress return true if is invalid didAddress
func IsInValidDIDAddress(address, chainID []byte) bool {
	pre := append([]byte(DIDPrefix), chainID...)
	pre = append(pre, ':')
	return !bytes.HasPrefix(address, pre)
}
