...
type Subscription interface {
	...
	// Err returns error when subscription been closed.
	Err() chan error
}
...
func (mux *TypeMux) Post(ev interface{}) error {
	...
	for _, handler := range mux.handlerList {
		handler(ev)
	}
	subs := mux.subm[rtyp]
	mux.mutex.RUnlock()
...
//AddHandler add event handler for audit event
func (mux *TypeMux) AddHandler(f func(interface{})) {
	mux.mutex.Lock()
	mux.handlerList = append(mux.handlerList, f)
	mux.mutex.Unlock()
}
