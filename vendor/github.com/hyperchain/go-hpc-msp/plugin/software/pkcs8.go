...
func ParsePKCS8PrivateKey(der []byte) (crypto.PrivateKey, error) {
	...
	case privKey.Algo.Algorithm.Equal(OidPublicKeyRSA):
		...
		mode, merr := ModeFromRSAMod(key.N.BitLen())
		if merr != nil {
			return nil, errors.New("x509: failed to parse RSA private Key embedded in PKCS#8: " + merr.Error())
		}
		return &PrivateKey{
			PublicKey: PublicKey{
				Mode: mode,
				Key:  (*asym.RSAPublicKey)(&key.PublicKey),
			},
			PrivKey: (*asym.RSAPrivateKey)(key),
		}, nil
	...
	case privKey.Algo.Algorithm.Equal(OidPublicKeyECDSAOrSM2):
		...
		return &PrivateKey{
			PublicKey: PublicKey{
				Mode: common.ModeFromCurve(key.Curve),
...
func MarshalPKCS8PrivateKey(key *PrivateKey) ([]byte, error) {
	...
	case crypto.Rsa2048, crypto.Rsa3072, crypto.Rsa4096:
		...
		privKey.PrivateKey = MarshalPKCS1PrivateKey((*rsa.PrivateKey)(key.PrivKey.(*asym.RSAPrivateKey)))
