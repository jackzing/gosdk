...
type CertificateRequest struct {
	Raw                      []byte // Complete ASN.1 DER derEncode (CSR, signature algorithm and signature).
	RawTBSCertificateRequest []byte // Certificate request info part of raw ASN.1 DER derEncode.
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv crypto.SignKey) (csr []byte, err error) {
	...
	publicKeyAlgorithm, err = GetPublicKeyAlgorithmFromMode(priv.GetKeyInfo())
	if err != nil {
		return nil, err
	}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	out := &CertificateRequest{
		...
		PublicKeyAlgorithm: GetPublicKeyAlgorithmFromAlgorithmIdentifier(in.TBSCSR.PublicKey.Algorithm),
	...
	//parse vk
	var err error
	rawPub, mode, perr := ParsePKIXPublicKey(in.TBSCSR.PublicKey.Raw)
	if perr != nil {
		return nil, perr
	}
	out.PublicKey, err = parseDERPublicKey(rawPub, mode)
	if err != nil {
		return nil, err
	}
