...
// Sign sign a msg
func Sign(engine cryptocom.Engine, key crypto.SignKey, msg []byte) ([]byte, error) {
...
func Verify(engine cryptocom.Engine, key crypto.VerifyKey, msg, signature []byte) (bool, error) {
	var hasher crypto.Hasher
	var err error
	switch key.GetKeyInfo() {
	case crypto.Sm2p256v1:
