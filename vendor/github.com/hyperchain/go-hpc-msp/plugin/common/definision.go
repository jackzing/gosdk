...
//DummyCAName is a string for dummy CA
const DummyCAName = "CN=dummyCA"

//DummyID is a string for public Identity's Org
const DummyID = "dummyID"

//nolint
type (
	RandomFuncSlot        func() (io.Reader, error)
	HashFuncSlot          func(mode int) (crypto.Hasher, error)
	CryptFuncSlot         func(mode int, pwd, key []byte) (crypto.SecretKey, error)
	VerifyFuncSlot        func(key []byte, mode int) (crypto.VerifyKey, error)
	SignGetFuncSlot       func(keyIndex string) (crypto.SignKey, error)
	SignCreateFuncSlot    func() (index string, k crypto.SignKey, err error)
	ParseCertificateSlot  func(string) (crypto.Cert, error)
	ParseCASlot           func([]string) ([]crypto.CA, error)
	IssueSlot             func(ca crypto.CA, hostname string, ct crypto.CertType, ext map[string]string, vk crypto.VerifyKey) ([]byte, error)
	GenerateLocalCASlot   func(hostName string) (skIndex string, ca crypto.CA, err error)
	KeyAgreementInitSlot  func() (data1, data2ToPeer []byte, err error)
	KeyAgreementFinalSlot func(algo string, data1, data2FromPeer []byte) (crypto.SecretKey, error)
	VerifyAccSlot         func(mode int) (cryptocom.VerifyACCKey, error)
	//EncKeyFuncSlot        func(index []byte, mode int) (crypto.EncKey, error)
	//DecGetFuncSlot        func(keyIndex string) (crypto.DecKey, error)
	//DecCreateFuncSlot     func() (index string, k crypto.DecKey, err error)
)

//HashAlgoAll {FakeHash,SHA1,SM3,Sm3WithPublicKey, {SHA2,SHA3,KECCAK}X{Size224,Size256,Size384,Size512}},total=16
...
//SymAlgoAll {{AES, 3DES}X{CBC, ECB, GCM}, SM2_CBC} total=7
...
//AsymAlgoAll {Sm2p256v1, Secp256k1, Secp256r1, Secp256k1Recover, Secp384r1, Secp521r1} total=6
...
//DefaultPubForSelfTest default raw pub for self test
var DefaultPubForSelfTest = map[int][]byte{
...
//DefaultSignForSelfTest default sign for selfTest
var DefaultSignForSelfTest = map[int][]byte{
...
const (
	...
	Hash               //存在多种必须的算法，因此需要软件补位
	Crypt              //存在多种必须的算法，因此需要软件补位
	Signature          //verify (L0) 存在多种必须的算法
	SessionKey
	Accelerate
	FunctionMax
)
