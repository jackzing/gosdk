...
func Decode(pfxData []byte, password string) (privateKey interface{}, certificate *software.Certificate, err error) {
	var caCerts []*software.Certificate
...
func DecodeChain(pfxData []byte, password string) (privateKey interface{}, certificate *software.Certificate, caCerts []*software.Certificate, err error) {
	...
	for _, bag := range bags {
		...
		case bag.ID.Equal(oidCertBag):
			certsData, dErr := decodeCertBag(bag.Value.Bytes)
			if dErr != nil {
				return nil, nil, nil, dErr
			}

			certs, dErr := software.ParseCertificates(certsData)
			if dErr != nil {
				return nil, nil, nil, dErr
			}
			if len(certs) != 1 {
...
func Encode(
	...
	certificate *software.Certificate,
	caCerts []*software.Certificate,
	...
	kvb, err := encodePkcs8ShroudedKeyBag(rand, privateKey.(*software.PrivateKey).PrivKey, encodedPassword)
	if err != nil {
		return nil, err
	}
	keyBag := safeBag{
