...
type Method struct {
	...
	ParallelLevel parallel.ParallelLevel `json:"parallel_level"`
	MutexFields   []MutexField           `json:"mutex_fields"`
	MutexCalls    []MutexCall            `json:"mutex_calls"`
...
func (a *Abi) encodeMethod(methodName string, params ...interface{}) ([]byte, error) {
	if methodId, ok := a.methodIndex[methodName]; ok {
		...
		for i := 0; i < len(method.Input); i++ {
			...
			a.curParaIndex = append(a.curParaIndex, paraIndex{
				start: buf.Len(),
				end:   buf.Len() + len(tmp) - 1,
			})
			if err != nil {
				return nil, err
			}
