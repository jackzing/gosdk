...
func TestRPC_BVMCrossChainAnchorTx1(t *testing.T) {
	...
	//tx1.Sign(key)
	rp.namespace = "global"
	re1, err := rp.SignAndInvokeCrossChainContract(tx1, "invokeAnchorContract", key)
	assert.Nil(t, err)
...
func TestRPC_BVMCrossChainAnchorTx2(t *testing.T) {
	...
	rp.namespace = "ns2"
	re2, err := rp.SignAndInvokeCrossChainContract(tx2, "invokeAnchorContract", key)
	assert.Nil(t, err)
...
func TestRPC_BVMGetAnchorStatus(t *testing.T) {
	...
	//tx1.Sign(key)
	rp.namespace = "global"
	re1, err := rp.SignAndInvokeContract(tx1, key)
	assert.Nil(t, err)
	t.Log(bvm.Decode(re1.Ret))
}
...
func TestRPC_Hvm_Cross_Chain_Dep(t *testing.T) {
	...
	rp.namespace = "ns2"
	receipt, err := rp.SignAndDeployContract(transaction, key)
	assert.Nil(t, err)
...
func TestRPC_Hvm_Cross_Chain_SetTargetAddress(t *testing.T) {
	...
	rp.namespace = "ns1"
	invokeContract, err := rp.SignAndInvokeContract(transaction1, key)
	if err != nil {
		t.Error(err)
	}
...
func TestRPC_Hvm_Cross_Chain_Invoke(t *testing.T) {
	...
	transaction1 := NewTransaction(newAddress.Hex()).Invoke("0xd8b9d000225f5f636859e0bb5599ea833f349326", payload).VMType(HVM)
	rp.namespace = "ns1"
	invokeContract, err := rp.SignAndInvokeCrossChainContract(transaction1, "invokeContract", key)
	if err != nil {
		t.Error(err)
	}
	t.Log(invokeContract)
...
func TestRPC_Cross_Chain_Timeout(t *testing.T) {
	...
	assert.Nil(t, err)
	rp.namespace = "global"
	invoke, err := rp.SignAndInvokeCrossChainContract(tx, "invokeTimeoutContract", key)
	if err != nil {
		t.Error(err)
	}
	t.Log(invoke)
