...
// NewJsonRPC get RPC instance with default conf directory path "../conf"
func NewJsonRPC() (*RPC, error) {
	return NewJsonRPCWithPath(common.DefaultConfRootPath)
}

// NewJsonRPCWithPath get RPC instance with user defined root conf directory path
func NewJsonRPCWithPath(confRootPath string) (*RPC, error) {
	cf, err := config.NewFromFile(confRootPath)
	if err != nil {
		logger.Errorf("check config file, %s", err.Error())
		return nil, err
	}
...
// Deprecated: use NewJsonRPC instead
func NewRPC() *RPC {
...
func NewRPCWithPath(confRootPath string) *RPC {
	cf, err := config.NewFromFile(confRootPath)
	if err != nil {
		logger.Fatalf("check config file, %s", err.Error())
		panic(err)
	}
...
func newInspectorManager(cf *config.Config, confRootPath string) (im *inspectorManager) {
	...
	im.didAddress = cf.GetInspectorDidAddress()
	im.key = key
	return
}
...
func (rpc *RPC) jsonRPC(method string, params ...interface{}) *JSONRequest {
	...
	if rpc.im.enable {
		var addr string
		if len(rpc.im.didAddress) != 0 {
			if utils.IsHex(rpc.im.didAddress) {
				addr = rpc.im.didAddress
			} else {
				addr = utils.ToHex([]byte(rpc.im.didAddress))
			}
		} else {
			addr = rpc.im.key.GetAddress().Hex()
		}
		auth := &Authentication{
			Address:   addr,
			Timestamp: time.Now().UnixNano(),
		}
		sig, err := SignWithDID(rpc.im.key, authNeedHash(auth), false, false, len(rpc.im.didAddress) != 0)
		if err != nil {
...
// CallByPolling call and get tx receipt by polling
func (rpc *RPC) callTransactionByPolling(method string, transaction *Transaction, param interface{}, isPrivateTx bool) (*TxReceipt, StdError) {
	var (
		req    *JSONRequest
		data   json.RawMessage
		hash   string
...
// Deprecated: use GetBlocksWithLimit instead for it will OOM when data is too large
...
func (rpc *RPC) GetTransactionsByBlkNumWithLimit(start, end uint64, metadata *Metadata) (*PageResult, StdError) {
	...
	data, err := rpc.call(method, qtr.Serialize())
	if err != nil {
		return nil, err
	}
...
func (rpc *RPC) GetTxAvgTimeByBlockNumber(from, to uint64) (uint64, StdError) {
	...
	data, err := rpc.call(method, mp.Serialize())
	if err != nil {
		return 0, err
	}
...
func (rpc *RPC) GetTxCountByContractAddr(from, to uint64, address string, txExtra bool) (*TransactionsCountByContract, StdError) {
	address = account.ConvertDIDAddrToHex(address)
...
func (rpc *RPC) GetTxByTimeAndContractAddrWithLimit(start, end uint64, metadata *Metadata, contractAddr string) (*PageTxs, StdError) {
	contractAddr = account.ConvertDIDAddrToHex(contractAddr)
	param := &IntervalTime{
...
func (rpc *RPC) GetTxConfirmedReceipt(txHash string) (*TxReceipt, StdError) {
	...
	data, err := rpc.call(method, txHash)
	if err != nil {
		return nil, err
	}
...
func (rpc *RPC) SendTx(transaction *Transaction) (*TxReceipt, StdError) {
	...
	return rpc.callTransactionByPolling(method, transaction, param, transaction.isPrivateTx)
}
...
func (rpc *RPC) GetContractCountByAddr(accountAddress string) (uint64, StdError) {
	accountAddress = account.ConvertDIDAddrToHex(accountAddress)
...
func (rpc *RPC) GetTransactionsByExtraID(extraId []interface{}, txTo string, detail bool, mode int, metadata *Metadata) (*PageResult, StdError) {
	...
	if txTo != "" {
		txTo = account.ConvertDIDAddrToHex(txTo)
		filter.TxTo = txTo
	}
...
func ValidateTxProof(txhash string, txroot string, path *TxProofPath) (bool, StdError) {
	txHash := utils.HexToBytes(txhash)
	txRooot := utils.HexToBytes(txroot)
	return types.ValidateMerkleProof(path.TxProof, txHash, txRooot), nil
}

// GetAccountProof query proofPath of given account.
func (rpc *RPC) GetAccountProof(ac string) (*AccountProofPath, StdError) {
	ac = account.ConvertDIDAddrToHex(ac)
	method := PROOF + "getAccountProof"
	data, err := rpc.call(method, ac)
	if err != nil {
		return nil, err
	}
...
// GetBalance 获取账户余额
// Deprecated
func (rpc *RPC) GetBalance(act string) (string, StdError) {
	act = account.ConvertDIDAddrToHex(act)
	method := ACCOUNT + "getBalance"
	data, err := rpc.call(method, act)
	if err != nil {
		return "", err
	}
...
// GetRoles 获取账户角色
func (rpc *RPC) GetRoles(act string) ([]string, StdError) {
	act = account.ConvertDIDAddrToHex(act)
...
func (rpc *RPC) GetAccountStatus(act string) (string, StdError) {
	act = account.ConvertDIDAddrToHex(act)
...
func (rpc *RPC) SetAccount(key interface{}) {
	switch key.(type) {
	case account.Key:
		rpc.im.key = key.(account.Key)
		rpc.im.didAddress = ""
	case *account.DIDKey:
		didKey := key.(*account.DIDKey)
		rpc.im.key = didKey.GetNormalKey()
		rpc.im.didAddress = didKey.GetAddress()
	}
}
