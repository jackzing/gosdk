...
func TestRPC_FileUpload(t *testing.T) {
	...
	var err error
	txHash, err = rp.FileUpload(filePath, "des", userList, nodeIdList, nodeIdList, accountJson, password)
	assert.Nil(t, err)
	t.Log(txHash)
	_, rperr, _ := rp.GetTxReceiptByPolling(txHash, false)
	assert.Nil(t, rperr)
	deleteDir(t, dirPath)
}
...
func TestRPC_GetFileExtraByTxHash(t *testing.T) {
	...
	rp, err := NewJsonRPC()
	assert.Nil(t, err)
	fextra, err := rp.GetFileExtraByTxHash(txHash)
	assert.Nil(t, err)
	t.Log(fextra.Hash)
...
func TestRPC_FileDownloadByTxHash(t *testing.T) {
	...
	assert.Nil(t, err)
	// 文件hash需要使用在本地上传文件返回的hash
	downloadPath, err := rp.FileDownloadByTxHash(dirPath, txHash, 1, accountJson, password)
	assert.Nil(t, err)
	t.Log(downloadPath)
	deleteDir(t, dirPath)
}
...
func TestRPC_FilePush(t *testing.T) {
	...
	rp, err := NewJsonRPC()
	assert.Nil(t, err)
	_, err = rp.FilePush(fileHash, []int{1, 2, 3}, accountJson, password, 1)
	assert.Nil(t, err)
}
...
func TestRPC_DownloadAndUpload(t *testing.T) {
	...
	go func() {
		fuHash, err := rp.FileUpload(uploadPath, "des", userList, nodeIdList, nodeIdList, accountJson, password)
		assert.Nil(t, err)
		t.Log(fuHash)
		ch <- struct{}{}
	}()
	go func() {
		fmInfo, err := rp.FileDownload(dirPath, hash, addr.Hex(), 3, accountJson, password)
		assert.Nil(t, err)
		t.Log(fmInfo)
		ch <- struct{}{}
	}()
...
func TestRPC_ConcurrentDownload(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		go func(id int) {
			...
			fileDownloadTX.SetExtraIDString(hash)
			fileDownloadTX.Sign(fmKey)
			downloadPath, err := rp.FileDownload(dirPath, hash, addr.Hex(), id%3+1, accountJson, password)
			if err != nil {
...
func TestRPC_ConcurrentDownload2(t *testing.T) {
	...
	// 向多个节点上传文件
	for i := 0; i < uploadCount; i++ {
		fuHash, err := rp.FileUpload(fmt.Sprintf(filePath, i), "des", userList, []int{1, 2, 3, 4}, nil, accountJson, password)
		assert.Nil(t, err)
		_, err, _ = rp.GetTxReceiptByPolling(fuHash, false)
		assert.Nil(t, err)
	...
	for i := 0; i < downloadCount; i++ {
		go func(id int) {
			...
			addr := fmKey.GetAddress()
			finfo, err := rp.GetFileExtraByTxHash(hash)
			assert.NoError(t, err)
			downloadPath, err := rp.FileDownload(dirPath, finfo.Hash, addr.Hex(), id%4, accountJson, password)
			assert.Nil(t, err)
			t.Log(downloadPath)
			ch <- struct{}{}
		}(i)
...
func TestRPC_ConcurrentFileUpload(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		go func(id int) {
			...
			fuHash, err := rp.FileUpload(path, "des", userList, []int{1, 2, 3, 4}, nil, accountJson, password)
			assert.Nil(t, err)
			t.Log(fuHash)
			ch <- struct{}{}
		}(i)
...
func TestRPC_ConcurrentFileUpload2(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		go func(id int) {
			nodeId := 1
			fmInfo, err := rp.FileUpload(filePath, "des", userList, []int{nodeId}, nil, accountJson, password)
			assert.Nil(t, err)
			t.Log(fmInfo)
			ch <- struct{}{}
		}(i)
...
func TestRPC_AllProcess(t *testing.T) {
	...
	fuHash, err := rp.FileUpload(uploadPath, "des", userList, []int{nodeId}, nil, accountJson, password)
	assert.Nil(t, err)
	t.Log(fuHash)
	t.Log(fuHash)
	_, err, _ = rp.GetTxReceiptByPolling(fuHash, false)
	assert.Nil(t, err)
	finfo, err := rp.GetFileExtraByTxHash(fuHash)
	assert.Nil(t, err)
	fileHash := finfo.Hash

	t.Log("账户不在白名单中，开始文件下载流程")
	addr := fmKey.GetAddress()
	downloadInfo, err := rp.FileDownload(dirPath, fileHash, fmKey.GetAddress().Hex(), nodeId, accountJson2, password)
	assert.NotNil(t, err)
	t.Log(downloadInfo)

	t.Log("更新白名单")
	fileExtra, gerr := rp.GetFileExtraByExtraId(fileHash)
	assert.Nil(t, gerr)
	...
	downloadInfo, err = rp.FileDownload(dirPath, fileHash, fmKey.GetAddress().Hex(), 1, accountJson2, password)
	assert.Nil(t, err)
	t.Log(downloadInfo)
}
...
func TestRPC_FileDownloadBreakpointContinue(t *testing.T) {
	...
	assert.Nil(t, err)
	// 文件hash需要使用在本地上传文件返回的hash
	_, err = rp.FileDownload(dirPath, fileHash, fmKey.GetAddress().Hex(), 3, accountJson, password)
	assert.Nil(t, err)
