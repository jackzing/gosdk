...
func TestRPC_CNSSmoke_Mapping(t *testing.T) {
	...
	t.Run("SetCName_normal", func(t *testing.T) {
		// 合约地址addr1无别名
		_, err := rp.GetNameByAddress(addr1)
		assert.NotNil(t, err)
		// 新增映射关系成功
		result := setCName(addr1, name1, rp, t)
		assert.True(t, result.Success)
		na, err := rp.GetNameByAddress(addr1)
		assert.Nil(t, err)
	...
	t.Run("SetCName_one address_two_name", func(t *testing.T) {
		...
		result := setCName(addr1, name2, rp, t)
		assert.True(t, result.Success)
	...
	t.Run("SetCName_used_cName", func(t *testing.T) {
		...
		result := setCName(addr2, name1, rp, t)
		assert.True(t, result.Success)
		var res []*bvm.OpResult
		_ = json.Unmarshal([]byte(result.Ret), &res)
		// 新增失败，交易回执会返回error
	...
	t.Run("SetCName_Check_Address_length", func(t *testing.T) {
		re, err := createProposalForSetCName("0x0000fff1", "Check_Address_length", rp, t)
		assert.Nil(t, err)
...
func TestRPC_CNSSmoke_QueryTx(t *testing.T) {
	...
	t.Run("get_tx_receipt", func(t *testing.T) {
		...
		// get receipt
		t.Log(re.TxHash)
		receipt, err := rp.GetTxReceipt(re.TxHash, false)
		assert.Nil(t, err)
	...
	t.Run("tx_getTxsByTime", func(t *testing.T) {
		...
		// get txs by time
		pageResult, err := rp.GetTxByTimeWithLimit(uint64(start), uint64(end), &Metadata{
			PageSize: 10,
			Backward: false,
		})
		assert.Nil(t, err)
		...
		pageResult, err = rp.GetTxByTimeAndContractAddrWithLimit(uint64(start), uint64(end), &Metadata{
			PageSize: 10,
			Backward: false,
		}, addr1)
		assert.Nil(t, err)
	...
	t.Run("tx_getTxsByTime_invokeByTo", func(t *testing.T) {
		...
		assert.Equal(t, addr1, pageResult.Data[0].To)
		// get txs by time and contract name
		pageResult, err = rp.GetTxByTimeAndContractNameWithLimit(uint64(start), uint64(end), &Metadata{
			PageSize: 10,
			Backward: false,
		}, name1)
		assert.Nil(t, err)
	...
	t.Run("tx_getTxsCount", func(t *testing.T) {
		...
		countByAddr, err := rp.GetTxCountByContractAddr(1, block.Number, addr1, false)
		assert.Nil(t, err)

		// 查询类接口使用合约地址命名cName，通过rpc方法tx_getTransactionsCountByContractName查询区块区间交易数量
		countByName, err := rp.GetTxCountByContractName(1, block.Number, name1, false)
		assert.Nil(t, err)
...
func TestRPC_CNSSmoke_QueryCNS(t *testing.T) {
	...
	t.Run("get_tx_by_name", func(t *testing.T) {
		...
		addr, err := rp.GetAddressByName(name1)
		// 查询成功
		assert.Nil(t, err)
		...
		txs, err := rp.GetNextPageTxs(min, 0, min, max, 0, 10, false, addr1)
		assert.Nil(t, err)
		marshal, _ := json.Marshal(txs)

		// 通过rpc方法查询下一页接口tx_getNextPageTransactions使用合约命名name下发查询
		txs1, err := rp.GetNextPageTxsByName(min, 0, min, max, 0, 10, false, name1)
		assert.Nil(t, err)
...
func TestRPC_CNSSmoke_RPC(t *testing.T) {
	...
	t.Run("get_tx_by_blk_number", func(t *testing.T) {
		...
		txInfo, err := rp.GetTxByBlkNumAndIdx(tx.BlockNumber, tx.TxIndex)
		assert.Nil(t, err)
		assert.Equal(t, addr1, txInfo.To)
		assert.Equal(t, "", txInfo.CName)

		// get block number
		tx, err = rp.GetTransactionByHash(nameHash)
		assert.Nil(t, err)
	...
	t.Run("get_tx_by_blk_hash", func(t *testing.T) {
		...
		txInfo, err := rp.GetTxByBlkHashAndIdx(tx.BlockHash, tx.TxIndex)
		assert.Nil(t, err)
		assert.Equal(t, addr1, txInfo.To)
		assert.Equal(t, "", txInfo.CName)
	...
	t.Run("GetTxByHash", func(t *testing.T) {
		// get tx by to hash
		txInfo, stdErr := rp.GetTransactionByHash(toHash)
		assert.Nil(t, stdErr)
		assert.Equal(t, addr1, txInfo.To)
		assert.Equal(t, "", txInfo.CName)

		// get tx by name hash
		txInfo, stdErr = rp.GetTransactionByHash(nameHash)
		assert.Nil(t, stdErr)
		assert.Equal(t, common.Address{}.Hex(), txInfo.To)
		assert.Equal(t, name1, txInfo.CName)
	})
	...
	t.Run("tx_getBatchTransactions", func(t *testing.T) {
		infos, err := rp.GetBatchTxByHash([]string{toHash})
		assert.Nil(t, err)
		assert.Equal(t, 1, len(infos))
		assert.Equal(t, addr1, infos[0].To)
		assert.Equal(t, "", infos[0].CName)

		infos, err = rp.GetBatchTxByHash([]string{nameHash})
		assert.Nil(t, err)
		assert.Equal(t, 1, len(infos))
...
func TestRPC_CNSSmoke_Sync(t *testing.T) {
	...
	t.Run("setCName", func(t *testing.T) {
		result := setCName(addr2, name2, rp, t)
		assert.True(t, result.Success)
	})

	t.Run("checkSync", func(t *testing.T) {
		// check mapping
		name, err := rp.GetNameByAddress(addr2)
		assert.Nil(t, err)
...
func setCName(address, cName string, rp *RPC, t *testing.T) *bvm.Result {
	re, err := createProposalForSetCName(address, cName, rp, t)
	...
	return executeProposal(int(proposal.Id), t, rp)
}

func createProposalForSetCName(address, cName string, rp *RPC, t *testing.T) (*TxReceipt, StdError) {
	key, _ := account.NewAccountFromAccountJSON(accountJsons[0], pwd)
...
func executeProposal(id int, t *testing.T, rp *RPC) *bvm.Result {
	key, _ := account.NewAccountFromAccountJSON(accountJsons[0], pwd)
...
func initHashContract(t *testing.T, rp *RPC) {
	key, _ := account.NewAccountFromAccountJSON(accountJsons[0], pwd)
...
func TestRPC_GetNextPageTxs2(t *testing.T) {
	...
	rp, err := NewJsonRPC()
	assert.Nil(t, err)
	txs, stdError := rp.GetNextPageTxs(24, 0, 24, 26, 0, 10, true, "0x0000000000000000000000000000000000ffff01")
	assert.Nil(t, stdError)
	marshal, _ := json.Marshal(txs)
	t.Log(string(marshal))
	txs, stdError = rp.GetNextPageTxsByName(24, 0, 24, 26, 0, 10, true, "HashContract")
	assert.Nil(t, stdError)
	marshal1, _ := json.Marshal(txs)
	t.Log(string(marshal1))
	assert.Equal(t, string(marshal), string(marshal1))

	txs, stdError = rp.GetPrevPageTxs(26, 0, 24, 26, 0, 10, true, "0x0000000000000000000000000000000000ffff01")
	assert.Nil(t, stdError)
	marshal, _ = json.Marshal(txs)
	t.Log(string(marshal))
	txs, stdError = rp.GetPrevPageTxsByName(26, 0, 24, 26, 0, 10, true, "HashContract")
	assert.Nil(t, stdError)
	marshal1, _ = json.Marshal(txs)
	t.Log(string(marshal1))
	assert.Equal(t, string(marshal), string(marshal1))
