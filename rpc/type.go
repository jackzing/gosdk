...
type BlockRaw struct {
	...
	TxRoot       string           `json:"txRoot,omitempty"`       // tx root hash
	Invalid      bool             `json:"invalid,omitempty"`
	InvalidMsg   string           `json:"invalid_msg,omitempty"`
	Author       string           `json:"author,omitempty"`
	Signature    string           `json:"signature,omitempty"`
	QuorumCert   QuorumCert       `json:"quorum_cert,omitempty"`
}
...
type LedgerInfo struct {
	CommitInfo        ConsensusBlockInfo `json:"commit_info"`
	ConsensusDataHash string             `json:"consensus_data_hash"`
}

type LedgerInfoWithSignatures struct {
	Signatures map[string]string `json:"signatures"`
	LedgerInfo LedgerInfo        `json:"ledger_info"`
}

type VoteData struct {
	Proposed ConsensusBlockInfo `json:"proposed"`
	Parent   ConsensusBlockInfo `json:"parent"`
}

type QuorumCert struct {
	VoteData         VoteData                 `json:"vote_data"`
	SignedLedgerInfo LedgerInfoWithSignatures `json:"signed_ledger_info"`
}
...
func (b *BlockRaw) ToBlock() (*Block, StdError) {
	...
	return &Block{
		...
		TxRoot:       b.TxRoot,
		Invalid:      b.Invalid,
		InvalidMsg:   b.InvalidMsg,
		Author:       b.Author,
		Signature:    b.Signature,
		QuorumCert:   b.QuorumCert,
	}, nil
}
...
// Status will be the following results
// 1. archive running
// 2. archive failed
// 3. archive finished
// 4. archive rollback
// 5. "" (with error message)
...
// AccountProofPath represents the result returned by account proof query.
