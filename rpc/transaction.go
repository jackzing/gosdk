...
const (
	...
	// DefaultTxGasLimit default transaction gas limit
	DefaultTxGasLimit = 1000000000
	// DefaultTxGasLimitV2 flato gas limit
	...
	DefaultExpirationTime = int64(5 * time.Minute)
)

const (
	TxVersion25 string = "2.5"
	TxVersion30 string = "3.0"
	TxVersion36 string = "3.6"
)
...
func (t *Transaction) GetExpirationTimestamp() int64 {
	return t.expirationTimestamp
}
...
func NewTransaction(from string) *Transaction {
	from = account.ConvertDIDAddrToHex(from)
	return &Transaction{
		timestamp:           getCurTimeStamp(),
		expirationTimestamp: getCurTimeStamp() + DefaultExpirationTime,
		nonce:               getRandNonce(),
		to:                  "0x0",
		from:                chPrefix(from),
		simulate:            false,
...
func (t *Transaction) ExpirationTimestamp(expirationTimestamp int64) *Transaction {
	t.expirationTimestamp = expirationTimestamp
	return t
}

func (t *Transaction) GasPrice(gasPrice int64) *Transaction {
	t.gasPrice = gasPrice
	return t
}

func (t *Transaction) GasLimit(gasLimit int64) *Transaction {
	t.gasLimit = gasLimit
	return t
}

func (t *Transaction) Participant(participant *Participant) *Transaction {
	t.participant = participant
	return t
}

func (t *Transaction) isDIDParticipant() bool {
	if t.participant == nil {
		return false
	}
	if strings.HasPrefix(string(t.participant.Initiator), account.DIDPREFIX) {
		return true
	}
	for _, v := range t.participant.Withholding {
		if strings.HasPrefix(string(v), account.DIDPREFIX) {
			return true
		}
	}
	return false
...
func (t *Transaction) Transfer(to string, value int64) *Transaction {
	to = account.ConvertDIDAddrToHex(to)
	t.value = value
	t.to = chPrefix(to)
	t.isValue = true
...
// DeployStringArgs add transaction deploy string args (args should be string or []interface{})
...
func (t *Transaction) Invoke(to string, payload []byte) *Transaction {
	if len(payload) > 8 && string(payload[0:8]) == "fefffbce" {
...
func (t *Transaction) InvokeBVM(to string, opt bvm.BuiltinOperation) *Transaction {
	if bvm.IsSystemOpt(opt) {
		t.GasPrice(math.MaxInt64)
		t.ExpirationTimestamp(math.MaxInt64)
	}
	payload := bvm.EncodeOperation(opt)
	t.vmType = string(BVM)
	return t.Invoke(to, payload)
}

// InvokeByName add transaction isInvoke
...
func (t *Transaction) MaintainDID(to string, op int64) *Transaction {
	t.to = account.ConvertDIDAddrToHex(to)
	t.isDID = true
	t.opcode = op
	return t
}
...
// SetIsDID set did flag
func (t *Transaction) SetIsDID(isDID bool) {
	t.isDID = isDID
}
...
func (t *Transaction) NeedHashString() string {
	return needHashString(t)
}
...
func (t *Transaction) GetTransactionHash(gasLimit int64) string {
	if CompareTxVersion(t.txVersion, types.TxVersion35) > 0 {
		var txHash [32]byte
		sig := common.FromHex(t.signature)
		binary.BigEndian.PutUint64(txHash[0:types.TimeLength], uint64(t.timestamp))
		copy(txHash[types.TimeLength:], sig[len(sig)-types.TimeLength*3:])
		return "0x" + common.Bytes2Hex(txHash[:])
	}
	...
	input := &protos.TransactionValue{
		Price:    defaultGasPrice,
		GasLimit: gasLimit,
		Amount:   t.value,
		Op:       protos.TransactionValue_Opcode(t.opcode),
		Extra:    []byte(t.extra),
	...
	case EVM:
		input.VmType = protos.TransactionValue_EVM
	case JVM:
		input.VmType = protos.TransactionValue_JVM
	case HVM:
		input.VmType = protos.TransactionValue_HVM
	case BVM:
		input.VmType = protos.TransactionValue_BVM
	case TRANSFER:
		input.VmType = protos.TransactionValue_TRANSFER
	case KVSQL:
		input.VmType = protos.TransactionValue_KVSQL
	case FVM:
		input.VmType = protos.TransactionValue_FVM
	default:
		return ""
	}
	if CompareTxVersion(t.txVersion, TxVersion36) >= 0 {
		input.Price = t.gasPrice
		input.GasLimit = t.gasLimit
	}
	...
	if CompareTxVersion(t.txVersion, TxVersion36) >= 0 {
		hashList = append(hashList, t.expirationTimestamp)
		if t.participant != nil {
			hashList = append(hashList, t.participant)
		}
	}
