...
const (
	...
	IssueBalance     ContractMethod = "Issue"

	// SystemUpgrade system upgrade proposal contract method `SystemUpgrade`
	SystemUpgrade ContractMethod = "SystemUpgrade"

	// GetLatestVersions version contract method `GetLatestVersions`
	GetLatestVersions ContractMethod = "GetLatestVersions"
	// GetSupportedVersionByHostname version contract method `GetSupportedVersionByHostname`
	GetSupportedVersionByHostname ContractMethod = "GetSupportedVersionByHostname"
	...
	versionAddress      = "0x0000000000000000000000000000000000ffff0c"
	hashAlgoAddress     = "0x0000000000000000000000000000000000ffff0d"
	issueBalanceAddress = "0x0000000000000000000000000000000000ffff0e"
...
const (
	...
	ProposalTypeCA
	// ProposalTypeSystem proposal of system type
	ProposalTypeSystem
)
...
type VersionOperationImpl struct {
	operationImpl
}

func (vo *VersionOperationImpl) Address() string {
...
// SystemUpgradeOperation defines system upgrade operation
type SystemUpgradeOperation interface {
	ProposalContentOperation
}

type systemUpgradeOperationImpl struct {
	operationImpl
}

func (suo *systemUpgradeOperationImpl) ProposalType() {}
...
// NewProposalCreateOperationForSystemUpgrade new proposal create operation for system upgrade operation
func NewProposalCreateOperationForSystemUpgrade(ops ...SystemUpgradeOperation) BuiltinOperation {
	data := encodeProposalContentOperation(convertToProposalContentOperations(ops))
	return newProposalCreateOperation(data, ProposalTypeSystem)
}
...
// NewPermissionGrantOperation create a new PermissionGrant operation and return
...
func NewSystemUpgradeOperation(targetRunningVersion types.RunningVersion) (SystemUpgradeOperation, error) {
	if targetRunningVersion == nil {
		return nil, errors.New("target running version cannot be empty")
	}
	rversionsBytes, merr := json.Marshal(targetRunningVersion)
	if merr != nil {
		return nil, merr
	}
	return &systemUpgradeOperationImpl{operationImpl{
		method: SystemUpgrade,
		args:   []string{string(rversionsBytes)},
	}}, nil
}

// NewGetLatestVersionsOperation creates a new GetLatestVersions operation and returns.
// It is used to query from ledger to get now running version and available version.
func NewGetLatestVersionsOperation() BuiltinOperation {
	return &VersionOperationImpl{operationImpl{
		method: GetLatestVersions,
	}}
}

// NewGetSupportedVersionByHostnameOperation creates a new GetSupportedVersionByHostname operation and returns.
// It is used to query from ledger to get supported version of the given node.
func NewGetSupportedVersionByHostnameOperation(hostname string) BuiltinOperation {
	return &VersionOperationImpl{operationImpl{
		method: GetSupportedVersionByHostname,
...
type IssueBalanceImpl struct {
	operationImpl
}

func (i IssueBalanceImpl) Address() string {
	return issueBalanceAddress
}

func newIssueBalanceOperation(method ContractMethod, args ...string) *IssueBalanceImpl {
	return &IssueBalanceImpl{operationImpl{method: method, args: args}}
}

// NewIssueBalanceSetOperation create a new IssueBalance set operation and return
func NewIssueBalanceSetOperation(addr string, value int64) BuiltinOperation {
	return newIssueBalanceOperation(IssueBalance, addr, strconv.FormatInt(value, 10))
}

func IsSystemOpt(opt BuiltinOperation) bool {
	switch opt.(type) {
	case *ProposalOperationImpl, *AccountOperationImpl, *CertOperationImpl,
		*DIDOperationImpl, *HashAlgoOperationImpl, *IssueBalanceImpl:
		return true
	case *RootCAOperationImpl:
		rcp := opt.(*RootCAOperationImpl)
		return rcp.Method() == AddRootCA
	}
