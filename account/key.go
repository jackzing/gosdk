...
func (key *ECDSAKey) GetAddress() common.Address {
	...
	h, err := hash.NewHasher(hash.KECCAK_256).Hash(bs[1:])
	if err != nil {
		return common.Address{}
	}
...
func (didKey *DIDKey) PrivateBytes() ([]byte, error) {
	return didKey.Key.PrivateBytes()
}

func ConvertDIDAddrToHex(addr string) string {
	if strings.HasPrefix(addr, DIDPREFIX) {
		return hexutil.Encode([]byte(addr))
	}
	return addr
}
