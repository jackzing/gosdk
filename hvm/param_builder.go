...
func NewParamBuilder(s string) *ParamBuilder {
	...
	p.paramIndex = make([]int, 0)
	methodName := []byte(s)
	p.buf.Write([]byte{254, 255, 251, 206}) // invokeDirectMagic
...
func (p *ParamBuilder) ParamIndex() []int {
	return p.paramIndex[:]
}
...
func (p *ParamBuilder) appendPayload(clazzName []byte, param []byte) {
	...
	// history problem: start and end need add 4
	// previous NewParamBuilder: p.buf.Write([]byte("fefffbce")) and hex payload without fefffbce
	// now: p.buf.Write([]byte{254, 255, 251, 206})) and hex payload together
	p.paramIndex = append(p.paramIndex, start+4, end+4)
}
