...
func TestRPC_MethodAbi(t *testing.T) {
	logger := common.GetLogger("main")
	methodInvokeJar := "../hvmtestfile/methodInvoke/sbank-1.0-bank.jar"
	methodInvokeAbi := "../hvmtestfile/methodInvoke/sbank-hvm.abi"
	hrpc := rpc.NewRPCWithPath("../conf")
	...
	// invokeBean
	beanAbi, err := abi.GetBeanAbi("invoke.InvokeBank")
	assert.Nil(t, err)
	invokePayload, err := GenPayload(beanAbi, "AAA", "BBB", 100)
	assert.Nil(t, err)
	...
	// method
	beanAbi1, err := abi.GetMethodAbi("transfer")
	assert.Nil(t, err)
	invokePayload1, err := GenPayload(beanAbi1, "AAA", "CCC", 200)
	assert.Nil(t, err)
	...
	// method
	beanAbi2, err := abi.GetMethodAbi("withdraw")
	assert.Nil(t, err)

	invokePayload2, err := GenPayload(beanAbi2, "CCC", 300)
	assert.Nil(t, err)
