- [第一章 前言](#第一章-前言)
  - [1.1 配置](#11-配置)
    - [1.1.1 目录结构](#111-目录结构)
    - [1.1.2 配置文件](#112-配置文件)
    - [1.1.3 配置项释义](#113-配置项释义)
- [第二章 账户](#第二章-账户)
  - [2.1 普通账户](#21-普通账户)
    - [2.1.1 创建账户](#211-创建账户)
    - [2.1.2 生成key](#212-生成key)
  - [2.2 did账户](#22-did账户)
    - [2.2.1 创建账户](#221-创建账户)
    - [2.2.2 生成key](#222-生成key)
- [第三章 交易体](#第三章-交易体)
  - [3.1 交易体结构](#31-交易体结构)
  - [3.2 交易体构造](#32-交易体构造)
    - [3.2.1 设置nonce](#321-设置nonce)
    - [3.2.2 设置交易体时间戳](#322-设置交易体时间戳)
    - [3.2.3 设置是否为Simulate交易](#323-设置是否为simulate交易)
    - [3.2.4  设置交易对应虚拟机类型](#324--设置交易对应虚拟机类型)
    - [3.2.5 转账交易构造](#325-转账交易构造)
    - [3.2.6 合约管理交易构造](#326-合约管理交易构造)
    - [3.2.7 通过合约名管理合约的交易构造](#327-通过合约名管理合约的交易构造)
    - [3.2.8 部署合约交易构造](#328-部署合约交易构造)
    - [3.2.9 带初始化参数的部署合约交易体构造](#329-带初始化参数的部署合约交易体构造)
    - [3.2.10 带字符串初始化参数的部署合约交易体构造](#3210-带字符串初始化参数的部署合约交易体构造)
    - [3.2.11 调用合约交易体构造](#3211-调用合约交易体构造)
    - [3.2.12 调用kvsql合约交易体构造](#3212-调用kvsql合约交易体构造)
    - [3.2.13 通过合约名字调用合约交易体构造](#3213-通过合约名字调用合约交易体构造)
    - [3.2.14 设置extra](#3214-设置extra)
    - [3.2.15 设置kvextra](#3215-设置kvextra)
    - [3.2.16 设置to](#3216-设置to)
    - [3.2.17 设置payload](#3217-设置payload)
    - [3.2.18 设置value](#3218-设置value)
    - [3.2.19 设置opcode](#3219-设置opcode)
    - [3.2.20 注册DID账户的交易体构造](#3220-注册did账户的交易体构造)
    - [3.2.21 管理DID账户生命周期的交易体构造](#3221-管理did账户生命周期的交易体构造)
    - [3.2.22 更新DID账户publickey的交易体构造](#3222-更新did账户publickey的交易体构造)
    - [3.2.23 更新DID账户的管理员账户交易体构造](#3223-更新did账户的管理员账户交易体构造)
    - [3.2.24 上传DID账户凭证交易体构造](#3224-上传did账户凭证交易体构造)
    - [3.2.25 下载DID账户凭证交易体构造](#3225-下载did账户凭证交易体构造)
    - [3.2.26 销毁DID账户凭证交易体构造](#3226-销毁did账户凭证交易体构造)
    - [3.2.27 本地计算交易体hash](#3227-本地计算交易体hash)
    - [3.2.28 设置DID账户的Extra交易体构造](#3228-设置did账户的extra交易体构造)
    - [3.2.29 获取DID账户的Extra交易体构造](#3229-获取did账户的extra交易体构造)
    - [3.2.30 调用bvm合约交易体构造](#3230-调用bvm合约交易体构造)
  - [3.3 交易体签名](#33-交易体签名)
    - [3.3.1 签名](#331-签名)
    - [3.3.2 批量签名](#332-批量签名)
    - [3.3.3 聚合签名](#333-聚合签名)
- [第四章 虚拟机](#第四章-虚拟机)
  - [4.1 EVM](#41-evm)
    - [4.1.1 abi解析](#411-abi解析)
    - [4.1.2 abi v2](#412-abi-v2)
  - [4.2 BVM](#42-bvm)
    - [4.2.1 编解码](#421-编解码)
    - [4.2.2 提案的使用](#422-提案的使用)
    - [4.2.3 hash的使用](#423-hash的使用)
    - [4.2.4 账户的使用](#424-账户的使用)
    - [4.2.5 证书的使用](#425-证书的使用)
    - [4.2.6 DID的使用](#426-did的使用)
    - [4.2.7 MPC的使用](#427-mpc的使用)
    - [4.2.8 Root CA的使用](#428-root-ca的使用)
    - [4.2.9 跨分区交易](#429-跨分区交易)
    - [4.2.10 hash类型查询](#4210-hash类型查询)
    - [4.2.11 Balance管理](#4211-balance管理)
    - [4.2.12 系统升级版本管理](#4212-系统升级版本管理)
  - [4.3 HVM](#43-hvm)
    - [4.3.1 abi解析](#431-abi解析)
    - [4.3.2 jar包部署](#432-jar包部署)
  - [4.4 KVSQL](#44-kvsql)
    - [4.4.1 回执解码](#441-回执解码)
  - [4.5 FVM](#45-fvm)
    - [4.5.1 abi编解码](#451-abi编解码)
- [第五章 RPC 接口](#第五章-rpc-接口)
- [第六章 GRPC 接口](#第六章-grpc-接口)
- [第七章 报错说明](#第七章-报错说明)
- [第八章 示例](#第八章-示例)
  - [8.1 EVM](#81-evm)
    - [8.1.1 编写合约代码](#811-编写合约代码)
    - [8.1.2 编译合约](#812-编译合约)
    - [8.1.3 使用gosdk](#813-使用gosdk)
  - [8.2 HVM](#82-hvm)
    - [8.2.1 HVM合约invokeBean 如下所示](#821-hvm合约invokebean-如下所示)
    - [8.2.2 HVM 合约接口定义以及合约实现如下所示：](#822-hvm-合约接口定义以及合约实现如下所示)
    - [8.2.3 合约编译以及ABI生成](#823-合约编译以及abi生成)
    - [8.2.4 调用上述合约的 gosdk代码如下所示](#824-调用上述合约的-gosdk代码如下所示)
    - [8.2.5 HVM合约并行调用](#825-hvm合约并行调用)
  - [8.3 KVSQL](#83-kvsql)
  - [8.4 BVM](#84-bvm)
  - [8.5 TRANSFER](#85-transfer)
  - [8.6 FVM](#86-fvm)
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/012d7bc7e95269239d1f14cfd8ee0ce4039b?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTY2Nzc4NjI4MiwiaWF0IjoxNjY3MTgxNDgyLCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMmQ3YmM3ZTk1MjY5MjM5ZDFmMTRjZmQ4ZWUwY2U0MDM5YiJ9.fbVhM3RqdjU5If6Gw2fFDLV1TEyCMI_9w2aOkCwuuZE&download=image.png "")
...
| grpc      | ports                 | 各节点grpc服务端口                                                  | ["11001", "11002", "11003", "11004"]              |
...
|           | maxRecvMsgSize        | grpc双向流从节点接受的信息最大size，此处配置必须小于等于节点配置的值，单位是比特                 | 51200                                             |
|           | maxSendMsgSize        | 通过grpc双向流发送到节点的信息最大size，此处配置必须小于等于节点配置的值，单位是比特               | 51200                                             |
|           | dailTimeout           | 建立grpc连接的超时时间，单位s                                            | 5                                                 |
|           | maxLifetime           | 判定接收不到grpc心跳多久后表示连接已失效，单位s， 如果为0，表示使用默认值                     | 0                                                 |
|           | maxStreamLifeTime     | grpc双向流的存活时间，此处配置必须小于等于节点配置的值，0表示默认（即依靠节点配置的时间），单位秒          | 5                                                 |
...
1. **hash相关， **HashContract合约地址为: `0x0000000000000000000000000000000000ffff01`

1. **did相关**， 合约地址为`0x0000000000000000000000000000000000ffff06`

1. **证书相关**， 合约地址为`0x0000000000000000000000000000000000ffff05`

1.  **账户相关**， 合约地址为`0x0000000000000000000000000000000000ffff04`

1. **mpc(多方安全计算)合约相关**，合约地址为`0x0000000000000000000000000000000000ffff09`

1. **Root CA相关**，合约地址为`0x0000000000000000000000000000000000ffff0b`

1. **跨分区相关**, 合约地址为`0x0000000000000000000000000000000000ffff0a`

1. **hash类型查询**， 合约地址为 `0x0000000000000000000000000000000000ffff0d`

1. **balanceManager相关**，合约地址为 `0x0000000000000000000000000000000000ffff0e`
...
![](http://teambitiondoc.hyperchain.cn:8099/storage/011n1ce4dcb6e2ba169962cd86345bc12c03?Signature=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJRCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9hcHBJZCI6IjU5Mzc3MGZmODM5NjMyMDAyZTAzNThmMSIsIl9vcmdhbml6YXRpb25JZCI6IiIsImV4cCI6MTY2Nzc4NjI4MiwiaWF0IjoxNjY3MTgxNDgyLCJyZXNvdXJjZSI6Ii9zdG9yYWdlLzAxMW4xY2U0ZGNiNmUyYmExNjk5NjJjZDg2MzQ1YmMxMmMwMyJ9.TGRdqO5uMOpOtmslOO471GU7LWYsCy41iaDpgtS80lM&download=%E6%8F%90%E6%A1%88%E7%8A%B6%E6%80%81%E8%BF%81%E7%A7%BB.png "")
...
- 内置合约名称：`VersionContract`，用于链级版本的管理。 
- 内置合约地址：0x0000000000000000000000000000000000ffff0c 
- API接口：通过API `contract_invokeContract`来调用该内置合约，VM类型为BVM。
- 内置合约方法：

VersionContract 内置合约的对外方法定义如下：

| 方法名                           | 参数                  | 返回值         | 作用                                          |
| ----------------------------- | ------------------- | ----------- | ------------------------------------------- |
| GetLatestVersions             | 无                   | \<bvmResult\> | 查询链运行版本信息，包括RunningVersion和AvailableVersion |
| GetSupportedVersionByHostname | \<string\>：节点hostname | \<bvmResult\> | 查询指定节点支持的版本信息SupportedVersion            |


**4.2.12.1 查询链运行版本**
- 方法描述：查询链运行版本信息，包括RunningVersion和AvailableVersion
- 合约内容构造函数：

```go
// NewGetLatestVersionsOperation 用于创建内置合约版本查询操作
func NewGetLatestVersionsOperation() BuiltinOperation {}
```
...
payload := bvm.EncodeOperation(operation)
tx := NewTransaction(key.GetAddress().Hex()).Invoke(operation.Address(), payload).VMType(BVM)
receipt, err := rp.SignAndInvokeContract(tx, key)
if err != nil {
	return err
}
// 解析交易回执结果
...
// 可以通过 string(bvmResult.Ret) 获取json结果的字符串表示
// 也可以通过json反序列化获取结构化结果
// maxSupportedVersions 表示节点支持的各链细分版本的最大版本号，key为节点hostname
//    availableVersions 表示当前可用于链下一次升级的链细分版本
//      runningVersions 表示当前运行的链细分版本
var vr struct {
...
**4.2.12.2 查询指定节点的支持版本**
- 方法描述：查询指定节点支持的版本信息SupportedVersion
- 合约内容构造函数：

```go
// NewGetSupportedVersionByHostnameOperation 用于创建内置合约的版本查询操作
// 入参 hostname 为指定要查询的节点hostname
func NewGetSupportedVersionByHostnameOperation(hostname string) BuiltinOperation {}
```

应用实例：

```go
// 构造并发送交易，例子为查询node1的可支持版本信息
rpc, err := NewJsonRPC()
...
[http://thoughts.hyperchain.cn:8099/workspaces/5b6c3babbe825b41b446a307/docs/5ed787cac98cc900016b726e](http://thoughts.hyperchain.cn:8099/workspaces/5b6c3babbe825b41b446a307/docs/5ed787cac98cc900016b726e)
...
goSDK支持HVM合约的并行调用，HVM并行合约的编写请参考HVM并行合约使用手册，这里主要给出使用并行合约的goSDK代码示例。

在GoSDK中原本就需要基于abi来进行HVM合约的调用，我们针对HVM并行调用提供了一个新的接口。

通过GenParallelPayload方法，可以获取到包含互斥变量信息和合约调用信息的payload。

```go
// abiGetter       根据合约地址获取合约对应的ABI
// addr            需要调用的合约地址
// beanName        调用类型（直接调用/InvokeBean调用）
// invokeDirectly  是否是直接调用
// params          调用参数
func GenParallelPayload(abiGetter AbiGetter, addr types.Address, 
  beanName string, invokeDirectly bool, params ...interface{}) 
  (payload []byte, err error)
```

AbiGetter是一个接口类型，提供了基于合约地址获取HVMAbi的方法。开发者可以选择GoSDK提供的默认实现AbiMap，也可以选择自己实现该接口。

```go
// AbiGetter get abi by contract address.
type AbiGetter interface {
   GetAbi(addr string) (Abi, error) // type.Address hex to abi
}
```

得到调用参数编码payload，通过Invoke方法构建hvm并行交易：
...
1. 创建abiMap对象，并将需要用到的abi保存到abiMap中。需要注意的是，如果本次调用涉及跨合约调用，请将被跨合约调用的合约ABI保存在abiMap。如果要调用的合约数量较少，可以将所有合约的abi保存在abiMap中。

1. 传入调用的方法名/InvokeBean名以及相关调用参数。可以获取到包含互斥变量信息和合约调用信息的payload。

1. 调用Invoke方法构建并行交易

```go
// 1. 事先读取abi文件，根据abi数据创建abi对象
abiPath := "../hvmtestfile/staticanalyze/parallel-contract.abi"
abiJson, _ := common.ReadFileAsString(abiPath)
abi0, _ := hvm.GenAbi(abiJson)
// 2. 创建abiMap的一个对象，并将需要用到的abi保存到abiMap中
abiMap := hvm.NewAbiMap()
abiMap.SetAbi(contractAddr, abi0)
// 3. 传入调用的方法名/invokeBean名 以及相关参数。
// 得到包含互斥变量信息和调用参数信息的payload
beanName := "demo.invoke.Transfer"
payload, _ := hvm.GenParallelPayload(abiMap, addr, beanName, false, "k1", "k2", 100)
// 4. hvm并行交易交易构建
